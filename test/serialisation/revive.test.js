...
test('#fromJSON(json)', async t => {
  ...
  await t.test('has no side-effects', async t => {
    let serialised, revived, constructed = null

    await t.test('subsequent revivals', async t => {
      t.beforeEach(() => {
        serialised = new Gate()
      })
      
      await t.test('return distinct instances', t => {
        revived = Gate.fromJSON(JSON.stringify(serialised))
          
        t.assert.strictEqual(revived.state, 'locked')
      })
    })
    
    await t.test('subsequent constructions', async t => {
      t.beforeEach(() => {
        constructed = new Gate()
        constructed.transition('unlock')
      })
      
      await t.test('return distinct instances', t => {
        t.assert.strictEqual(constructed.state, 'unlocked')
        t.assert.notStrictEqual(revived.state, constructed.state)
      })
    })
  })
})
