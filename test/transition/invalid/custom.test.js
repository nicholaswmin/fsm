...
test('#transitionFn() customising invalid transition behavior', async t => {
  ...
  await t.test('attemping an invalid transition', async t => {     
    ...
    // @FIXME this behavior is wrong, configuring `onInvalid` should only
    // apply on subsequent instances, not previous ones.
    await t.skip('instances created previously to method override', async t => {
      let previous = new FSM({ 
        closed: { coin: 'opened' },
        opened: { push: 'closed' }
      }, hooks), previousReturnedValue = null

      let current = null, currentReturnedValue = null
      ...
      t.before(() => { 
        ...
        previousReturnedValue = previous.push()
        currentReturnedValue = current.push()
      })

      t.after(restoreOnInvalid)

      await t.test('previous instance returns result of default method', t => {  
        t.assert.strictEqual(previousReturnedValue, false)
      })
      
      await t.test('new instance returns result of overriden method', t => {  
        t.assert.strictEqual(currentReturnedValue, 'foo')
      })
      
      await t.test('previous and new return different results', t => {  
        t.assert.notStrictEqual(previousReturnedValue, currentReturnedValue)
      })
    })
  })
})
