const vString = (v, name) => {
  const err = typeof v === 'undefined' 
    ? new TypeError(`"${name}" is missing`)
    : typeof v === 'string'
      ? v.length > 0
        ? null
...
const vInit = v => {
  const err = typeof v === 'undefined' 
    ? new TypeError('"init" parameter missing')
    : null
...
const vActionsPresent = (v, actions = {}) => {
...
const vActionsUtilised = (v, states = {}) => {
  const inStateActions = v => {
...
const initExists = (init, states) => {
  if (!Object.keys(states).includes(init))
    throw new RangeError(`"init" state: "${init}" is not a valid state`)
}

const vActionsMatchStates = (states, actions) => {
  vActionsPresent(states, actions)
